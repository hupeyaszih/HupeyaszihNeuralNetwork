...
int clear_time_window(TimeTable* time_table,int time_window_id, int new_target_time){
    ...
    for(int i = 0;i < time_table->time_windows[time_window_id].event_count;i++){
        int id = i + time_table->time_windows[time_window_id].event_start_idx;
        time_table->event_list->time[id] = -1;
        time_table->event_list->neuron_id[id] = -1;
    }
    return NN_SUCCESS;
}
